...
import {get, post} from '../method/fetch'
import {actionsTypes as IndexActionTypes} from '../reducers'

export function* login(phoneNum, password) {
    yield put({type: IndexActionTypes.FETCH_START});
    try {
        return yield call(post, '/user/login', {phoneNum, password})
    } catch (error) {
        yield put({type:IndexActionTypes.SET_MESSAGE,msgContent:'手机号或密码错误',msgType:0});
    } finally {
...
export function* loginFlow() {
    while (true) {
        ...
        let response = yield call(login, request.phoneNum, request.password);
